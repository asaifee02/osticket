 ...
 <table class="form_table fixed" width="940" border="0" cellspacing="0" cellpadding="2">
    ...
    <tbody>
        ...
        <tr><td><?php echo __('User'); ?>:</td><td>
            ...
            <!-- Custom code to comment Change button -->
            <?php /*
            ...
            <td>
                <select name="topicId" onchange="javascript:
                    ...
                    if ($topics=Topic::getHelpTopics(false, false, true)) {
                        ...
                        <option value="" selected >&mdash; <?php echo __('Select Product'); ?> &mdash;</option>
<?php                   }
        ...
        <!-- Custom code to select support owner -->
        <?php if($user){ ?>
        <tr>
            <td class="required"><?php echo __('Support Owner');?>:</td>
            <td>
                <select id="masterId" name="masterId">
                    <?php
                        $whitelist = array('127.0.0.1', '::1');
                        if(in_array($_SERVER['REMOTE_ADDR'], $whitelist)){//Localhost: 
                            $master_user_ids = MaximumTicketsPlugin::getMasterUserIdsByUserIdWithActiveSupports($user->getId());
                        }else{//Server
                            $master_user_ids = MaximumTicketsPlugin::getMasterUserIdsByUserIdWithActiveSupports_STAFF($user->getId());
                        }
                        if(sizeof($master_user_ids) == 0){
                            echo '<option value="" selected="selected">&mdash;No active support found &mdash;</option>';
                        }
        ...
        <!-- Custom code to show/hide priority type -->
        <style type="text/css">
            #dynamic-form tr:nth-child(5){   display:none; }
        </style>
        <!-- Custom code to show/hide priority type -->
        <tbody>
        <?php
