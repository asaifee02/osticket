...
implements RestrictedAccess, Threadable {
    ...
    function checkUserAccess($user) {
        ...
        //Custom code to show tickets of all team members
        $ticket_owner = MaximumTicketsPlugin::getUserIdByTicketId($this->getId());
        $master_user_ids = MaximumTicketsPlugin::getMasterUserIdsByUserId( $user->getId() );
        if (in_array($ticket_owner, $master_user_ids))
            return true;

        $support_ids = MaximumTicketsPlugin::getAllSupportIdsByUserId($user->getId());
        if(empty($support_ids))
            $support_ids[] = 0;
        $sql = "SELECT ps_id FROM ost_ticket WHERE ticket_id = ".$this->getId();
        $res = db_query($sql);
    ...
    /* Product title  - NOT object -> $topic */
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function emailStaffFunc($vars, $origin='', $alerts=true) {
        global $cfg;
        ...
        if($cfg->alertONNewMessage()
            ...
            if($dept && ($manager=$dept->getManager()))
                $recipients[]=$manager;
        ...
        $sql = "SELECT `id` FROM `ost_thread_entry` order by `id` desc limit 1";
        $res = db_query($sql);
        if($res){
            $row = db_fetch_row($res);
            $sql = "DELETE FROM `ost_thread_entry` WHERE `id`=".$row[0];
            db_query($sql);
        }
        return true;
    }
    ...
    function postReply($vars, &$errors, $alert=true, $claim=true) {
        ...
        //Custom Code to add signature in response
        $dept = $this->getDept();
        ...
        elseif($vars['signature']=='dept' && $dept && $dept->isPublic())
            ...
            $signature='';
        if(!empty($signature) OR $signature!=''){
            $temp = "<span style='color:rgb(127, 127, 127)'><hr> ".$signature."</span>";
            $vars['response'] .= $temp; 
        }
        //Custom Code to add signature in response
    ...
    function update($vars, &$errors) {
        ...
        $fields = array();
        $fields['topicId']  = array('type'=>'int',      'required'=>1, 'error'=>__('Product selection is required'));
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        switch (strtolower($origin)) {
            case 'web':
                $fields['topicId']  = array('type'=>'int',  'required'=>1, 'error'=>__('Select a Product'));
                // Custom code to validate support owner
                $fields['masterId']  = array('type'=>'int',  'required'=>1, 'error'=>__('Select a Support Owner'));
                // Custom code to validate support owner
                break;
            case 'staff':
        ...
        if (!$errors) {
            ...
            // Handle the forms associate with the Products. Instanciate the
            ...
            if ($vars['topicId']) {
                if ($__topic=Topic::lookup($vars['topicId'])) {
                    foreach ($__topic->getForms() as $idx=>$__F) {
                        ...
                        // requested to be disabled as per the Product.
                        if ($__F->get('type') == 'T') {
                            foreach ($form->getFields() as $field) {
        ...
        if ($vars['topicId']) {
            if ($topic=Topic::lookup($vars['topicId'])) {
                ...
                $errors['topicId'] = 'Invalid Product selected';
            }
        }
        ...
        // If the filter specifies a Product which has a form associated,
        // and there was previously either no Product set or the help
        ...
        // and filled out from the original  Product, and (3) staff
        ...
        // trump Product settins if the email has an associated help
        // topic
