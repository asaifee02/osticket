...
class osTicketSession {
    ...
    function __construct($ttl=0){
        ...
        // Custom code to fix new cookie rules issue, remove this change in php 7.3 and use built in options.
        ...
        session_start();
        // Custom code to fix new cookie rules issue, remove this change in php 7.3 and use built in options.
        if (osTicket::is_https()) { $params = "Secure; SameSite=none";}else{$params="";}
        header("Set-Cookie: OSTSESSID=".session_id().";Max-Age=".$ttl."; path=/; domain=".$domain."; HttpOnly;".$params);
        // Custom code to fix new cookie rules issue, remove this change in php 7.3 and use built in options.
    }
