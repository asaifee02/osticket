...
define('SECRET_SALT','08jrriKX77PtatV3w6z1B=4h=YYrU5Sb');

#Default admin email. Used only on db connection issues and related alerts.
define('ADMIN_EMAIL','assad.mahmood@aspose.com');
...
if(in_array($_SERVER['REMOTE_ADDR'], $whitelist)){//Localhost
    define('DBSITES', 'dynabic_osticket_data_sites');    
    define('DBPASS','1234');
}
else{//Server
    define('DBSITES', 'dynabic_osticket_sites');    
    define('DBPASS','a2387as');
}
//Custom Code
define('DBTYPE','mysql');
...
$sql = "select * from ost_sites where status=1 AND url = '$url'";
$res = $conn->query($sql);
$row = $res->fetch_array();
