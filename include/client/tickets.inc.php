...
if(isset($_GET['display_all']) AND $_GET['display_all']=="yes"){    
    $visibility = $basic_filter->copy()
        ->values_flat('ticket_id')
        ->filter(array('user_id' => $thisclient->getId()));

    //Display tickets of all master users
    $master_user_ids = MaximumTicketsPlugin::getMasterUserIdsByUserId($thisclient->getId());
    foreach ($master_user_ids as $master_user_id) {
        $visibility1 = $basic_filter->copy()
            ->values_flat('ticket_id')
            ->filter(array('user_id' => $master_user_id ));
        $visibility = $visibility -> union($visibility1, false);
    }

    //Display all tickets which were created by using current user's support orders
    $support_ids = MaximumTicketsPlugin::getAllSupportIdsByUserId($thisclient->getId());
    foreach ($support_ids as $support_id) {
        $visibility1 = $basic_filter->copy()
            ->values_flat('ticket_id')
            ->filter(array('ps_id' => $support_id ));
        $visibility = $visibility -> union($visibility1, false);
    }
}
else{
...
<form action="tickets.php" method="get" id="ticketSearchForm">
    ...
    <!-- Custom code to show tickets of all team members -->
    <?php 
    if($_GET['display_all']=='yes')
        echo '<input type="hidden" name="display_all" value="yes">';
    ?>
    <!-- Custom code to show tickets of all team members -->
...
<div class="pull-right">
    ...
    <select name="topic_id" class="nowarn" onchange="javascript: this.form.submit(); ">
        <option value="">&mdash; <?php echo __('All Products');?> &mdash;</option>
<?php
...
<div class="pull-right states">
    <small>
<!-- Custom code to show tickets of all team members -->
<?php if(isset($_GET['display_all']) AND $_GET['display_all']=="yes"){ ?>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="tickets.php"><button>Display My Tickets</button></a>
<?php } else {?>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="tickets.php?display_all=yes"><button>Display All Tickets</button></a>
<?php }?>
<!-- Custom code to show tickets of all team members -->
<?php if ($openTickets) { ?>
...
<table id="ticketTable" width="800" border="0" cellspacing="0" cellpadding="0">
    ...
    <thead>
        <tr>
            ...
            <!-- Custom code to show tickets of all team members -->
            <th width="170">
                <a href="tickets.php?sort=dept&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Department"><?php echo __('Owner');?></a>
            </th>
            <!-- Custom code to show tickets of all team members -->
        </tr>
    </thead>
     ...
     if ($tickets->exists(true)) {
         foreach ($tickets as $T) {
            ...
            <tr id="<?php echo $T['ticket_id']; ?>">
                ...
                <td>
                    <?php                         
                        $sql1 = "SELECT user_id FROM ".TICKET_TABLE." WHERE ticket_id = ".$T['ticket_id'];
                        $res1 = db_query($sql1);
                        $row1 = db_fetch_row($res1);
                        $user_id1 = $row1[0];
                        $sql1 = "SELECT name FROM ".USER_TABLE." WHERE id = ".$user_id1;
                        $res1 = db_query($sql1);
                        $row1 = db_fetch_row($res1);
                        echo $row1[0];
                    ?>
