...
implements TemplateVariable {
    ...
    /**
     * Determine if the Product is currently enabled. The ancestry of
    ...
    function update($vars, &$errors) {
        ...
        if (!$vars['topic'])
            $errors['topic']=__('Product name is required');
        ...
        if ($this->__new__) {
            ...
            if (!($rv = $this->save())) {
                $errors['err']=sprintf(__('Unable to create %s.'), __('this Product'))
               .' '.__('Internal error occurred');
            }
        }
        elseif (!($rv = $this->save())) {
            $errors['err']=sprintf(__('Unable to update %s.'), __('this Product'))
            .' '.__('Internal error occurred');
        }
        if ($rv) {
...
Filter::addSupportedMatches(/* @trans */ 'Product', array('topicId' => 'Topic ID'), 100);
