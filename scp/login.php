...
if(in_array($_SERVER['REMOTE_ADDR'], $whitelist)){//Localhost
    ...
    //Custom code to redirect to Passport Auth
    if (isset($_SESSION['_staff']['auth']['msg2'])) {
    }
    elseif ($_SERVER['QUERY_STRING'] != "do=ext&bk=dynabic.passport.staff")
        header("Location: http://".$_SERVER['HTTP_HOST']."/scp/login.php?do=ext&bk=dynabic.passport.staff");
    //Custom code to redirect to Passport Auth
}
//Custom Code for Passport Integration
...
elseif ($_GET['do']) {
    ...
    //Http::redirect('login.php');//Custom Code for Passport Integration (commented out this line)
}
