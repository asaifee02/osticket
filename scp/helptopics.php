...
if($_REQUEST['id'] && !($topic=Topic::lookup($_REQUEST['id'])))
    $errors['err']=sprintf(__('%s: Unknown or invalid ID.'), __('Product'));

if($_POST){
    switch(strtolower($_POST['do'])){
        case 'update':
            if(!$topic){
                $errors['err']=sprintf(__('%s: Unknown or invalid'), __('Product'));
            }elseif($topic->update($_POST,$errors)){
                $msg=sprintf(__('Successfully updated %s.'),
                    __('this Product'));
            }elseif(!$errors['err']){
                $errors['err'] = sprintf('%s %s',
                    sprintf(__('Unable to update %s.'), __('this Product')),
        ...
        case 'create':
            ...
            if ($_topic->update($_POST, $errors)) {
                ...
                $errors['err']=sprintf('%s %s',
                    sprintf(__('Unable to add %s.'), __('this Product')),
                    __('Correct any errors below and try again.'));
            }
            break;
        case 'mass_process':
                        ...
                        if ($num > 0) {
                            if($num==$count)
                                $msg = sprintf(__('Successfully enabled %s'),
                                    _N('selected Product', 'selected Products', $count));
                            else
