...
if($_POST && !$errors):
    ...
    if($ticket && $ticket->getId()) {
        ...
        case 'jiraissue': /* Post Jira Issues */
            $vars = $_POST;
            $ticketId = $vars['id'];
            $jiraIssues = $vars['jira-issue'];
            $pageUrl = $vars['pageurl'];
            if(empty($jiraIssues)) {
                DynabicJiraPlugin::deleteIssue($ticketId);
                $msg=__('Issues deleted successfully');
            } else { 
                $issues = explode(';', $jiraIssues);
                foreach($issues as $issueKey) {
                    $errorIssues .= DynabicJiraPlugin::addIssue($ticketId, $issueKey, $pageUrl);
                }
                //$ticket->updateJiraIssue($ticketId, $jiraIssues);
                if(isset($errorIssues) AND !empty($errorIssues)) {
                    $errors['err'] = __('Unable to verify the following issues');
                    $errors['jiraissue'] = $errorIssues;
                } else {
                    $msg=__('Issues added successfully');
                }   
            }
            break;
        //Custom Code for Jira Plugin
        //Custom Code for  Redmine Plugin
        case 'redmineissue': /* Post Redmine Issues */
            $vars = $_POST;
            $ticketId = $vars['id'];
            $jiraIssues = $vars['redmine-issue'];
            $pageUrl = $vars['pageurl'];
            if(empty($jiraIssues)) {
                DynabicRedminePlugin::deleteIssue($ticketId);
                $msg=__('Issues deleted successfully');
            } else { 
                $issues = explode(';', $jiraIssues);
                foreach($issues as $issueKey) {
                    $errorIssues .= DynabicRedminePlugin::addIssue($ticketId, $issueKey, $pageUrl);
                }
                //$ticket->updateJiraIssue($ticketId, $jiraIssues);
                if(isset($errorIssues) AND !empty($errorIssues)) {
                    $errors['err'] = __('Unable to verify the following issues');
                    $errors['redmineissue'] = $errorIssues;
                } else {
                    $msg=__('Issues added successfully');
                }   
            }
        ...
        switch($_POST['a']) {
            case 'open':
                ...
                if (!$thisstaff ||
                    ...
                    $vars = $_POST;
                    $_SESSION['parent_child'] = $vars['masterId'];//Custom code to show tickets of all team members
