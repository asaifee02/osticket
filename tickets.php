<?php
//Custom Code to fix confirm form resubmission popup during refreshing the page
...
$attachments2 = $messageField->getWidget()->getAttachments();
//Custom code to email staff 
...
if ($_POST && is_object($ticket) && $ticket->getId()) {
    ...
    case 'email':
        ...
        if(!$errors) {
            ...
            if(($msgid=$ticket->emailStaffFunc($vars, 'Web'))) {
...
if($ticket && $ticket->checkUserAccess($thisclient)) {
    if (isset($_REQUEST['a']) && $_REQUEST['a'] == 'edit'
        ...
        $inc='view.inc.php';
} else { //Custom code: Edited this code to allow sub-accounts to see all tickets even if their own ticket count is zero
    $inc='tickets.inc.php';
}
